...
function createParticleEffect(flowerType = null) {
  ...
  if (
    ...
    colors = ["#ff4d6d", "#ff758f", "#ff8e53", "#c9184a", "#ffccd5"]; // Pink tones (legendary)
  } else if (
    flowerType &&
    ...
    colors = ["#ff4081", "#ff758f", "#ff85a1", "#fb6f92"]; // Pink palette (rare)
  } else if (
    flowerType &&
    ...
    colors = ["#ffccd5", "#ffb3c1", "#ff85a1", "#ff758f"]; // Light pink shades (uncommon)
  } else {
    colors = ["#ff4d6d", "#ff758f", "#ff85a1", "#fb6f92"]; // Default colors (common)
  }
...
function renderCollection() {
  ...
  Object.entries(categories).forEach(([category, flowerTypes]) => {
    ...
    flowerTypes.forEach((type) => {
      ...
      collectionItem.className = `collection-item ${data.count > 0 ? "unlocked" : "locked"
        } ${category}`;
...
function createConfetti() {
  ...
  const colors = ["#ff4d6d", "#ff758f", "#ff4081", "#c9184a", "#ffccd5"];
...
function acceptLove() {
  const assurance = document.getElementById('assuranceScreen');
  assurance.classList.add('hidden');
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  setTimeout(() => {
    ...
    // Show assurance screen after loading
    setTimeout(() => {
      const assurance = document.getElementById('assuranceScreen');
      assurance.classList.remove('hidden');
    }, 500);
  }, 3000);
...
if (typeof createConfetti !== 'function') {
  window.createConfetti = function () {
    const colors = ['#ff4d6d', '#ff758f', '#ffffff', '#ffccd5'];
    for (let i = 0; i < 50; i++) {
